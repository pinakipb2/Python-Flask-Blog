...
app.config.update(
    ...
    MAIL_PORT = '465',
    MAIL_USE_SSL = True,
    MAIL_USENAME = params['gmail_user'],
...
class Posts(db.Model):
    ...
    tn_img_file = db.Column(db.String(60), nullable=True)
...
def home():
    ...
    if(not str(page).isnumeric()):
        page = 1
    page = int(page)
    if(int(params['no_of_posts'])<len(posts)) :
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            tn_img_file = request.form.get('tn_img_file')
            date = datetime.now()

            if sno=='0':
                post = Posts(title=box_title, slug=slug, content=content, img_file=img_file, tn_img_file=tn_img_file, date=date)
                db.session.add(post)
                db.session.commit()
